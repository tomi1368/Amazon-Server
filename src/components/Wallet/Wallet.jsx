...
const Wallet = () => {
  ...
  return (
    ...
    <div className="checkout-container__checkout">
        ...
        { user && !cart.products.length == 0 ? <button onClick={()=> productCheckOut()  }> Procced to Checkout </button> : <button style={{backgroundColor:"#cccccc"}} > {cart.products.length == 0 ? "Empty Cart" : "Login to Checkout"} </button> }
    </div>
    </div>
    </div>
  );
